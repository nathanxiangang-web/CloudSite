...
The current reviewed baseline records **2 exact debt IDs**:
...
| `router_orm_import` | 2 | A legacy router still imports SQLAlchemy, shared ORM models, or the legacy database layer directly. |
...
and to 35 when content-root path validation and root-mapping CRUD moved behind the Providers contract,
and to 33 when Catalog publication-scope list/toggle/public DTO and sitemap ownership moved behind the Catalog contract,
and to 31 when role policy and User role query/update workflows moved behind the Users contract,
...
The latest ratchet moved 8 -> 6 by removing lazy ORM/SQLAlchemy reads from the admin Quality router.\n\nThe latest ratchet moved 6 -> 4 by removing direct ORM/SQLAlchemy access from the admin Catalog router.\n\nThe latest ratchet moved 4 -> 2 by making Setup a workflow module, moving wizard ORM ownership behind its contract, and routing Provider/Presentation writes through owner boundaries.\n\nThe latest ratchet moved 4 -> 2 when Setup became a workflow module that owns wizard progress and orchestrates Providers/Presentation boundaries instead of importing ORM in the router.
...
```text
90 -> 88 -> 87 -> 86 -> 85 -> 84 -> 83 -> 82 -> 81 -> 79 -> 76 -> 71 -> 67 -> 59 -> 58 -> 55 -> 52 -> 51 -> 50 -> 49 -> 45 -> 41 -> 39 -> 37 -> 35 -> 33 -> 31 -> 29 -> 27 -> 23 -> 19 -> 17 -> 14 -> 12 -> 10 -> 8 -> 6 -> 4 -> 2 -> ... -> 0
```
