...
def test_select_orders_by_priority():
    ...
    normal = _candidate(path='/normal', depth=2, last_verified_at=_NOW - timedelta(days=10))
    top = _candidate(path='/top', depth=0)
    result = selector.select([normal, failed, top], now=_NOW)
    assert result[0].path == '/failed'
    assert result[1].path == '/top'
    assert result[2].path == '/normal'
