...
def validate_download_url(value: str, expected_host: str | None = None) -> tuple[str, str]:
    if not value or any(ord(character) < 32 for character in value):
        raise DownloadError("DL-008", "download URL safety check failed", "url_validation")
    parsed = urlparse(value)
    if parsed.scheme not in {"http", "https"} or not parsed.hostname or parsed.username or parsed.password:
        raise DownloadError("DL-008", "download URL safety check failed", "url_validation")
    if expected_host and parsed.hostname.lower() != expected_host.lower():
        raise DownloadError("DL-008", "download entry safety check failed", "redirect_validation")
    return value, parsed.hostname


def map_alist_error(exc: Exception) -> DownloadError:
    if not isinstance(exc, AListError):
        return DownloadError("DL-999", "download service temporarily unavailable", "download_entry")
    if exc.status_code == 429:
        return DownloadError("DL-003", "cannot read AList file info", "alist_file_info", 429)
    if exc.code == "AL-002":
        return DownloadError("DL-002", "AList temporarily unreachable", "alist_connection", 503)
    if exc.code in {"AL-003", "AL-004"}:
        return DownloadError("DL-006", "AList authentication failed", "authentication", 503)
    if exc.code == "AL-005":
        return DownloadError("DL-003", "cannot read AList file info", "alist_file_info", 503)
    return DownloadError("DL-999", "download service temporarily unavailable", "download_entry")
...
async def resolve_download_entry(resource, connection) -> DownloadResolution:
    ...
    if not connection or not connection.enabled:
        raise DownloadError("DL-002", "AList not configured or disabled", "alist_connection", 503)
