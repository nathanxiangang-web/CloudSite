"""Resources-owned persistent download rate limiting."""
...
from .models import DownloadRateLimit
...
def hash_ip(address: str) -> str:
    ...
    return hmac.new(
        purpose_key,
        normalize_ip(address).encode("utf-8"),
        hashlib.sha256,
    ).hexdigest()
    ...
    try:
        ...
        for item in raw:
            if not isinstance(item, str):
                continue
            hits.append(
                _as_utc(datetime.fromisoformat(item.replace("Z", "+00:00")))
            )
        return hits
    ...
    async with lock:
        async with state_session() as session:
            ...
            row = await session.scalar(
                select(DownloadRateLimit).where(
                    DownloadRateLimit.ip_key == ip_key
                )
            )
            if row is None:
            ...
            if row.blocked_until and current < _as_utc(row.blocked_until):
                retry_after = max(
                    ...
                    math.ceil(
                        (
                            _as_utc(row.blocked_until) - current
                        ).total_seconds()
                    ),
                )
            ...
            window_start = current - timedelta(
                seconds=DOWNLOAD_RATE_WINDOW_SECONDS
            )
            hits = [
                hit
                for hit in _decode_hits(row.recent_hits_json)
                if hit > window_start
            ]
            if len(hits) < DOWNLOAD_RATE_MAX_ATTEMPTS:
                hits.append(current)
                row.recent_hits_json = json.dumps(
                    [
                        _encode_time(hit)
                        for hit in hits[-DOWNLOAD_RATE_MAX_ATTEMPTS:]
                    ]
                )
            ...
            row.blocked_until = current + timedelta(
                seconds=DOWNLOAD_RATE_BLOCK_SECONDS
            )
            row.recent_hits_json = json.dumps(
                [
                    _encode_time(hit)
...
async def cleanup_download_rate_limits(
    now: datetime | None = None,
) -> int:
    current = _as_utc(now or utcnow())
    ...
    async with state_session() as session:
        result = await session.execute(
            delete(DownloadRateLimit).where(
                ...
                (DownloadRateLimit.blocked_until.is_(None))
                | (DownloadRateLimit.blocked_until <= current),
            )
        )
        await session.commit()
...
def rate_limit_payload(decision: DownloadRateDecision) -> dict:
    return {
        ...
        "message": "download rate limited, please retry later",
        "retry_after": decision.retry_after,
        "blocked_until": (
            _encode_time(decision.blocked_until)
