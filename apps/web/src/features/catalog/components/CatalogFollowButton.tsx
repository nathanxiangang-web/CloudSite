...
const FOLLOW_STATUS_KEY = (userId: number | null, entryId: string) =>
  ["catalog-follow-status", userId, entryId] as const;
...
export function CatalogFollowButton({ entryId }: { entryId: string }) {
  ...
  if (auth.isLoading) {
    return (
      <span className="catalog-follow-button placeholder" aria-busy="true">
        …
      </span>
    );
  }
  ...
  if (!auth.data?.authenticated) {
    return (
      <Link
        className="button catalog-follow-button login-required"
        href={`/login?next=${encodeURIComponent(`/catalog/${entryId}`)}`}
      >
        <Star size={16} />
        登录后关注
      </Link>
