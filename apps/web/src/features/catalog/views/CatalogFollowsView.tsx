...
const FOLLOW_KEY = (userId: number | null) =>
  ["my-catalog-follows", userId] as const;

export function CatalogFollowsView() {
  const auth = useAuth();
  ...
  return (
    <div className="page account-library-page">
      ...
      {query.isLoading ? (
            ...
            <article key={item.entry_id}>
              ...
              <div className="account-follow-actions">
                <button
                  type="button"
                  aria-label={
                    item.notify_enabled ? "关闭更新通知" : "开启更新通知"
                  }
                  disabled={toggleNotify.isPending}
                  onClick={() =>
                    toggleNotify.mutate({
                      entryId: item.entry_id,
                      enabled: !item.notify_enabled,
                    })
                  }
                >
        ...
        <div className="empty">
          还没有关注任何资源条目。去 <Link href="/catalog">资源目录</Link>{" "}
          关注感兴趣的软件吧。
        </div>
      )}
    </div>
