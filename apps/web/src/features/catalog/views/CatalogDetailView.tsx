...
export function CatalogDetailView({
  ...
  if (entry.isLoading) {
    return <div className="page loading">正在加载目录条目…</div>;
  }
  if (entry.error) {
    return (
      ...
      <div className={styles.releaseTabs}>
        {sorted.map((item) => (
          <button
            key={item.release_id}
            type="button"
            className={item.release_id === activeReleaseId ? styles.selected : undefined}
            onClick={() => setReleaseId(item.release_id)}
          >
            <span>{item.title}</span>
            <span>{channelLabel(item.channel)}</span>
            {releaseIsRecommended(item) && <span>推荐</span>}
          </button>
        ))}
      </div>
          ...
          {filtered.map((asset) => (
            <article key={asset.asset_id} className="panel catalog-asset-card">
              ...
              <dl>
                ...
                <div>
                  <dt>大小</dt>
                  <dd>{asset.size != null ? formatBytes(asset.size) : "以实际下载为准"}</dd>
                </div>
                <div>
                  <dt>状态</dt>
              ...
              <form
                action={catalogAssetDownloadPath(entryId, asset.asset_id)}
                method="POST"
              ...
              >
                <button
                  ...
                  aria-disabled={asset.availability !== "available"}
                >
                  <Download size={16} />
                  {asset.availability === "available" ? "立即下载" : "暂不可用"}
                </button>
              </form>
            </article>
          ))}
