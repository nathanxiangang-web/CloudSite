...
import { ChevronLeft, ChevronRight, Download, File, FileText, Heart, Image as ImageIcon, Package2, RefreshCw, Share2, Video } from "lucide-react";
  ...
  return <PublicShell><div className="page detail-page">
    ...
    <section className="detail-grid">
      ...
      <aside className="detail-meta">
        ...
        <dl><div><dt>类型</dt><dd>{item.extension?.toUpperCase() || item.content_type}</dd></div><div><dt>大小</dt><dd>{formatBytes(item.size)}</dd></div><div><dt>更新时间</dt><dd>{item.modified_at ? new Date(item.modified_at).toLocaleString("zh-CN") : "未知"}</dd></div></dl>
