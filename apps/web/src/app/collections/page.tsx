...
export default function CollectionsPage() {
  ...
  const countText = query.data ? `${collections.length} 个合集` : query.isLoading ? "正在读取合集…" : "合集数量不可用";

  return <PublicShell><div className="page collection-page">
    <section className="library-hero">
      <span className="library-folder type-video"><FolderKanban /></span>
      <div><h1>精选合集</h1><p>跨目录整理的主题资源，一次浏览完整内容。</p><div className="meta">{countText}</div></div>
    </section>
    <h2 className="subheading">全部合集</h2>
    {query.isLoading
      ? <div className="loading">正在加载合集…</div>
      : query.error
        ? <div className="empty error-state">合集暂时不可用：{query.error.message}<button type="button" onClick={() => query.refetch()}>重试</button></div>
        : collections.length
          ? <section className="collection-grid">{collections.map((collection, index) => <Link href={`/collections/${collection.id}`} className="collection-card" key={collection.id}>
            <div className="cover"><ImageAsset src={collectionCoverSrc(collection.cover, `/assets/collection-${(index % 4) + 1}.webp`)} alt={collection.name} fill sizes="(max-width:768px) 50vw, 25vw" /></div>
