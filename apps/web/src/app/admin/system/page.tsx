...
export default function SystemPage() {
  ...
  if (system.data !== syncedSystem) {
    ...
    if (system.data) setSystemForm({ automatic_sync: system.data.automatic_sync, sync_interval_minutes: normalizeSyncInterval(system.data.sync_interval_minutes), sync_on_startup: system.data.sync_on_startup });
  }
  ...
  return <AdminShell title="系统"><div className="admin-page">
    ...
    <section className="panel index-and-mapping"><h2><Database />索引状态</h2><dl><div><dt>资源总数</dt><dd>{system.data?.resources ?? 0}</dd></div><div><dt>文件夹数量</dt><dd>{system.data?.folders ?? 0}</dd></div><div><dt>操作日志</dt><dd>{system.data?.operation_logs ?? 0}</dd></div><div><dt>数据库</dt><dd>{system.data?.database ?? "SQLite 3"}</dd></div></dl>
    ...
    <section className="panel"><h2><Server />服务信息</h2><dl><div><dt>API 版本</dt><dd>v{system.data?.version ?? "—"}</dd></div><div><dt>部署模式</dt><dd>Docker Compose</dd></div><div><dt>时区</dt><dd>{system.data?.timezone ?? "Asia/Shanghai"}</dd></div></dl></section>
      </div>
    </div>}
  </div>
