...
type SystemSettings = { automatic_sync: boolean; sync_interval_minutes: number; sync_on_startup: boolean; version: string; database: string; timezone: string; resources: number; folders: number };
...
export default function SystemPage() {
  ...
  const [systemForm, setSystemForm] = useState({ automatic_sync: false, sync_interval_minutes: 60, sync_on_startup: false });
  ...
  return <AdminShell title="系统"><div className="admin-page system-grid">
    ...
    <section className="panel"><h2><RefreshCw />自动同步</h2><div className="setting-row"><span><strong>自动同步</strong><small>按间隔增量同步 AList 变化</small></span><input className="toggle" type="checkbox" checked={systemForm.automatic_sync} onChange={(e) => setSystemForm({ ...systemForm, automatic_sync: e.target.checked })} /></div><label className="select-label">同步间隔<select value={systemForm.sync_interval_minutes} onChange={(e) => setSystemForm({ ...systemForm, sync_interval_minutes: Number(e.target.value) })}><option value={15}>15 分钟</option><option value={30}>30 分钟</option><option value={60}>1 小时</option><option value={360}>6 小时</option><option value={1440}>24 小时</option></select></label><div className="setting-row"><span><strong>启动时同步</strong><small>容器启动后自动执行一次</small></span><input className="toggle" type="checkbox" checked={systemForm.sync_on_startup} onChange={(e) => setSystemForm({ ...systemForm, sync_on_startup: e.target.checked })} /></div><button className="primary" onClick={() => saveSystem.mutate()}><Save />保存同步设置</button></section>
    <section className="panel"><h2><Server />服务信息</h2><dl><div><dt>API 版本</dt><dd>v{system.data?.version ?? "0.1.0"}</dd></div><div><dt>部署模式</dt><dd>Docker Compose</dd></div><div><dt>健康状态</dt><dd className="ok-text">健康</dd></div><div><dt>时区</dt><dd>{system.data?.timezone ?? "Asia/Shanghai"}</dd></div></dl></section>
  </div>
