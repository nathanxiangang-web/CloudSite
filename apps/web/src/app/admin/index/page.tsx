...
export default function IndexPage() {
  ...
  return <AdminShell title="内容索引"><div className="admin-page index-admin-page">
    ...
    <section className="panel index-control-panel"><div><h2>动态索引</h2><p>目录和资源完全来自已启用的 AList 根目录映射。</p></div><div className="index-actions"><button type="button" disabled={busy} onClick={() => sync.mutate(false)}><RefreshCw className={busy ? "spin" : ""} />立即同步</button><button type="button" className="primary" disabled={busy} onClick={() => sync.mutate(true)}><RotateCcw />完整重建</button></div>{sync.error && <p className="form-error">{sync.error.message}</p>}</section>
