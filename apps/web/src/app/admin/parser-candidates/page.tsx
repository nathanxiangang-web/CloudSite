...
export default function ParserCandidatesPage() {
  ...
  const key = ["admin-parser-candidates", status, resourceFilter] as const;
  const candidates = useQuery({
    queryKey: key,
    queryFn: () => api<{ items: ParserCandidateTask[]; total_returned: number }>(parserCandidateListPath({ status, resourceId: resourceFilter.trim(), limit: 100 })),
  });
  ...
  return <AdminShell title="解析候选">
    <div className="admin-page parser-candidate-page">
      ...
      <section className="panel parser-candidate-list">
        <div className="panel-toolbar"><div><h2>候选任务</h2><p>{candidates.data?.total_returned ?? 0} 条</p></div><div className="parser-filters">
          <select aria-label="状态筛选" value={status} onChange={(event) => setStatus(event.target.value as ParserCandidateStatus | "")}>
            {STATUSES.map((value) => <option value={value} key={value || "all"}>{value ? parserCandidateStatusLabel(value) : "全部状态"}</option>)}
          </select>
          <input aria-label="资源筛选" placeholder="按资源 ID 筛选" value={resourceFilter} onChange={(event) => setResourceFilter(event.target.value)} />
        ...
        {candidates.isLoading ? <div className="loading">正在读取…</div>
          ...
          : candidates.data?.items.length ? <div>{candidates.data.items.map((task) => <CandidateRow task={task} onRetry={() => retry.mutate(task.task_id)} retrying={retry.isPending} key={task.task_id} />)}</div>
