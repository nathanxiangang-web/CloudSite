...
const matchTypeLabel: Record<string, string> = { path: "路径匹配", hash: "哈希匹配", size: "大小匹配", rename: "重命名推断", move: "移动推断", manual: "人工标注" };
const candidateStatusLabel: Record<string, string> = { open: "待确认", resolved: "已确认", ambiguous: "歧义", dismissed: "已忽略" };
...
export default function DiagnosticsPage() {
  ...
  return <AdminShell title="下载诊断"><div className="admin-page diagnostics-page">
    <section className="panel diagnostic-runner"><h2>测试 AList 原生下载链路</h2><p className="panel-intro">选择或输入资源 ID，服务端会检查索引、AList 文件信息、访问根目录、下载签名和安全跳转；页面只显示 AList 域名，不显示签名。</p>
      ...
      {keyword && <div className="diagnostic-suggestions">{search.isFetching ? <span>正在搜索…</span> : search.error ? <><span>搜索失败：{search.error.message}</span><button type="button" onClick={() => search.refetch()}>重试</button></> : search.data?.items.length ? search.data.items.map((item) => <button key={item.id} onClick={() => { setId(item.id); setKeyword(""); }}><strong>{item.name}</strong><small>{item.extension || item.content_type} · {item.id}</small></button>) : <span>没有匹配资源</span>}</div>}
      <div className="diagnostic-form"><input value={id} onChange={(event) => setId(event.target.value)} placeholder="资源 ID" /><label><input type="checkbox" checked={forceRefresh} onChange={(event) => setForceRefresh(event.target.checked)} />重新读取 AList 文件信息</label><button className="primary" disabled={!id.trim() || diagnose.isPending} onClick={() => diagnose.mutate()}>{diagnose.isPending ? "诊断中…" : "开始诊断"}</button></div>
    ...
    <section className="panel identity-diagnostics"><h2><Fingerprint />稳定资源 ID</h2>{identityStats.isLoading ? <div className="loading">正在读取身份注册表…</div> : identityStats.error ? <p className="form-error">{identityStats.error.message}</p> : <><div className="identity-stat-grid"><article><small>身份总数</small><strong>{identityStats.data?.total ?? 0}</strong></article><article><small>旧版保留</small><strong>{identityStats.data?.legacy_seeded ?? 0}</strong></article><article><small>新随机 ID</small><strong>{identityStats.data?.random_new ?? 0}</strong></article><article><small>重命名/移动保留</small><strong>{(identityStats.data?.rename_preserved ?? 0) + (identityStats.data?.move_preserved ?? 0)}</strong></article><article><small>待判定</small><strong>{identityStats.data?.pending ?? 0}</strong></article><article><small>歧义</small><strong>{identityStats.data?.ambiguous ?? 0}</strong></article></div><h3>待确认候选</h3>{identityCandidates.isLoading ? <div className="loading">正在读取候选…</div> : identityCandidates.error ? <p className="form-error">候选加载失败：{identityCandidates.error.message}</p> : identityCandidates.data?.items.length ? <div className="identity-candidate-list">{identityCandidates.data.items.map((item) => <article key={item.id}><div><strong>{item.observed_path}</strong><small>{labelOf(matchTypeLabel, item.match_type)} · {item.candidate_resource_ids.length} 个候选 · {new Date(item.created_at).toLocaleString("zh-CN")}</small></div><b>{labelOf(candidateStatusLabel, item.status)}</b></article>)}</div> : <div className="empty compact">当前没有待确认或歧义候选。</div>}</>}</section>

    <section className="panel diagnostic-history"><h2><Clock3 />最近诊断记录</h2>{history.isLoading ? <div className="loading">正在读取…</div> : history.error ? <div className="empty error-state">读取诊断记录失败：{history.error.message}<button type="button" onClick={() => history.refetch()}>重试</button></div> : history.data?.items.length ? history.data.items.map((item) => <article key={item.id}><span className={`diagnostic-dot ${item.status}`} /><div><strong>{item.resource_id}</strong><small>{item.message}{item.target_host ? ` · ${item.target_host}` : ""}</small></div><b>{item.error_code || `${item.duration_ms} ms`}</b><time>{new Date(item.created_at).toLocaleString("zh-CN")}</time></article>) : <div className="empty compact">还没有诊断记录。</div>}</section>
  </div></AdminShell>;
}
