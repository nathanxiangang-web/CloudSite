...
export default function AdminUsersPage() {
  ...
  return <AdminShell title="用户管理"><div className="admin-page">
    <section className="panel users-panel">
      ...
      {query.isLoading ? <div className="loading">正在读取用户…</div> : query.error ? <div className="empty error-state">加载失败：{query.error.message}<button type="button" onClick={() => query.refetch()}>重试</button></div> : query.data?.items.length ? query.data.items.map((user) => <div className="table-row users-table-row" key={user.id}>
    ...
    {selectedId !== null && <div className="user-modal-backdrop" role="presentation" onMouseDown={() => setSelectedId(null)}><section className="panel user-modal" role="dialog" aria-modal="true" aria-label="用户详情" onMouseDown={(event) => event.stopPropagation()}><button className="user-modal-close" onClick={() => setSelectedId(null)}><X /></button><h2>用户详情</h2>{detail.isLoading ? <div className="loading">正在读取…</div> : detail.error ? <div className="empty error-state">读取用户详情失败：{detail.error.message}<button type="button" onClick={() => detail.refetch()}>重试</button></div> : detail.data ? <dl className="user-detail-list"><div><dt>用户 ID</dt><dd>#{detail.data.id}</dd></div><div><dt>用户名</dt><dd>{detail.data.username}</dd></div><div><dt>密码</dt><dd aria-label="密码不可查看">......</dd></div><div><dt>状态</dt><dd>{detail.data.status === "active" ? "正常" : detail.data.status === "disabled" ? "已停用" : "已删除"}</dd></div><div><dt>创建方式</dt><dd>{detail.data.created_by_admin ? "管理员创建" : "用户注册"}</dd></div><div><dt>注册时间</dt><dd>{formatTime(detail.data.created_at)}</dd></div><div><dt>最近登录</dt><dd>{detail.data.last_login_at ? formatTime(detail.data.last_login_at) : "从未登录"}</dd></div><div><dt>密码更新时间</dt><dd>{detail.data.password_changed_at ? formatTime(detail.data.password_changed_at) : "从未修改"}</dd></div></dl> : null}</section></div>}
