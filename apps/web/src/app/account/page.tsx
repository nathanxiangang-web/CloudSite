...
import { CalendarDays, KeyRound, LogOut, ShieldCheck, UserRound } from "lucide-react";
  ...
  return <PublicShell><div className="page account-page">
    {!user ? <div className="loading">正在读取账号…</div> : <>
      ...
      <section className="account-grid">
        ...
        <article className="panel account-actions"><h2><CalendarDays />账号操作</h2><p>你可以更新登录密码，修改后其他设备上的旧会话会立即失效。</p><Link className="button primary" href="/account/security"><KeyRound />修改密码</Link><button type="button" disabled={logout.isPending} onClick={() => logout.mutate()}><LogOut />{logout.isPending ? "正在退出…" : "退出登录"}</button></article>
      </section>
    </>}
  </div></PublicShell>;
}
